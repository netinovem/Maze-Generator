        ...
        foreach (var mode in new[] { "background", "wall" })
        {
            foreach (var cell in EachCell())
            {
                    ...
                    if (backgroundColor != null)
                        image.Mutate(ctx =>
                            ctx.Fill(
                                ...
                                new RectangleF(x1, y1, cellSize, cellSize)
                            )
                        );
                }
                else //mode == "wall"
                {
                    ...
                    {
                        image.Mutate(ctx =>
                            ctx.DrawLine(Color.Black, 1, new PointF(x1, y1), new PointF(x2, y1))
                        );
                    }
                    if (cell.west == null) // No western link
                    {
                        image.Mutate(ctx =>
                            ctx.DrawLine(Color.Black, 1, new PointF(x1, y1), new PointF(x1, y2))
                        );
                    }
                    if (!cell.IsLinked(cell.east)) // Not linked to the east
                    {
                        image.Mutate(ctx =>
                            ctx.DrawLine(Color.Black, 1, new PointF(x2, y1), new PointF(x2, y2))
                        );
                    }
                    if (!cell.IsLinked(cell.south)) // Not linked to the south
                    {
                        image.Mutate(ctx =>
                            ctx.DrawLine(Color.Black, 1, new PointF(x1, y2), new PointF(x2, y2))
                        );
                    }
                }
            }
